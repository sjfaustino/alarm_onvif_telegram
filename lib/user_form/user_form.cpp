...
String renderTelegramUserForm(const TelegramUser& v, const std::vector<CameraConfig>& cams, bool isEdit) {
  ...
  String html = "<fieldset><legend>" + legend + "</legend><form method=\"POST\" action=\"/users/save\">";
  if (isEdit) html += htmlHiddenInput("originalName", v.name);
  html += htmlTextInput("Name (unique)", "name", v.name, " required");
  html += htmlTextInput("Chat ID (message @userinfobot, or check "
                        "https://api.telegram.org/bot&lt;TOKEN&gt;/getUpdates after messaging your bot)",
                        "chatId", v.chatId, " required");
  html += htmlCheckbox("All cameras (including ones added later)", "allCameras", v.allCameras);

  html += "<label>Or pick specific cameras (ignored if \"All cameras\" is checked):</label>";
  html += "<div class=\"camera-list\">";
  if (cams.empty()) html += "<span class=\"hint\">No cameras defined yet.</span>";
  for (auto& c : cams) {
    bool checked = false;
    for (auto& n : v.cameraNames) {
      if (n.equalsIgnoreCase(c.name)) { checked = true; break; }
    }
    html += htmlCheckbox(htmlEscape(c.name), "cam_" + c.name, checked);
  }
  html += "</div>";

  html += htmlCheckbox("Receive heartbeat and boot-online messages", "systemMessages", v.systemMessages);
  html += htmlCheckbox("May send /on, /off, /status, /uptime commands", "canCommand", v.canCommand);
  html += htmlCheckbox("May send /snap (on-demand photo) - independent of the commands above", "canSnap", v.canSnap);
  html += htmlCheckbox("May send /reset (reboots the board immediately) - independent of the permissions above, "
                       "off by default even for a new user",
                       "canReset", v.canReset);
  html += htmlCheckbox("May send /backup (sends the full config export as a file - includes every camera's own "
                       "username/password, though never the WiFi password) - independent of the permissions "
                       "above, off by default even for a new user",
                       "canBackup", v.canBackup);
  html += htmlCheckbox("May send /restore (uploads a config export file back to the bot and applies it, "
                       "overwriting cameras/users/network settings) - the single most disruptive command "
                       "available, independent of the permissions above, off by default even for a new user",
                       "canRestore", v.canRestore);
  html += htmlTextInput("Max commands per minute (0 = unlimited)", "maxCommandsPerMinute",
                        String(v.maxCommandsPerMinute));
  html += htmlSelect("Language (alerts and command replies sent to this user - the dashboard itself always "
                     "stays English)",
                     "language", {{"en", "English"}, {"pt", "Portugu\xC3\xAAs"}},
                     v.language == TelegramLang::Portuguese ? "pt" : "en");
