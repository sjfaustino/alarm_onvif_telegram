...
void test_trNtpSyncFailed_differs_by_hasFallbackTime_and_language(void) {
  String enWithFallback = trNtpSyncFailed(TelegramLang::English, true);
  String enNoFallback = trNtpSyncFailed(TelegramLang::English, false);
  // Must actually say something different, not ignore hasFallbackTime.
  TEST_ASSERT_TRUE(enWithFallback != enNoFallback);
  TEST_ASSERT_TRUE(enWithFallback.indexOf("fallback") >= 0);
  TEST_ASSERT_TRUE(enNoFallback.indexOf("fallback") >= 0);

  String ptWithFallback = trNtpSyncFailed(TelegramLang::Portuguese, true);
  TEST_ASSERT_TRUE(enWithFallback != ptWithFallback);
  TEST_ASSERT_TRUE(ptWithFallback.indexOf("alternativa") >= 0);
}
